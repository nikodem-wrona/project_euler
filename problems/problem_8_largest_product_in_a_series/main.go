package main

import (
	"fmt"
	"os"
	"strconv"
	"strings"
)

func splitToGroupsOf(data []string, sizeOfGroup int) [][]int {
	arrayOfGroups := [][]int{}

	for i := 0; i < len(data); i++ {
		group := []int{}

		start := i
		end := start + sizeOfGroup

		for j := start; j < end; j++ {
			if j >= len(data) {
				break
			}

			number, err := strconv.Atoi(data[j])

			if err != nil {
				fmt.Println(err)
				os.Exit(2)
			}

			group = append(group, number)
		}

		arrayOfGroups = append(arrayOfGroups, group)
	}

	return arrayOfGroups
}

func calculateProductsOfNumbersInGroups(groups [][]int) []int {
	results := []int{}

	for _, group := range groups {
		product := 1

		for _, x := range group {
			product *= x
		}

		results = append(results, product)
	}

	return results
}

func minMax(array []int) (int, int) {
	var max int = array[0]
	var min int = array[0]
	for _, value := range array {
		if max < value {
			max = value
		}
		if min > value {
			min = value
		}
	}
	return min, max
}

func main() {
	data := ""
	arrayOfNumbers := strings.Split(data, "")

	fmt.Println("Lenght of group : ", len(data))
	groups := splitToGroupsOf(arrayOfNumbers, 13)
	fmt.Println("Result number of groups : ", len(groups))

	products := calculateProductsOfNumbersInGroups(groups)
	fmt.Println("Number of products : ", len(products))

	_, largestProduct := minMax(products)

	fmt.Println("Largest product : ", largestProduct)
}
